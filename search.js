...
function showLoading() {
  ...
  // Show the small loading box
  const smLoadingBoxes = document.querySelectorAll(".sm_loading_box");
  smLoadingBoxes.forEach((box) => {
...
async function pollForAircraft(
  ...
  while (attempt < maxAttempts) {
    ...
    // Display main content (aircraft list, map) if searchResultsMatch is true OR maxAttempts reached
    if (
      !mainContentDisplayed &&
      (searchResultsMatch || attempt >= maxAttempts)
    ) {
      mainContentDisplayed = true;
      hideLoading(); // Hide loading screen when main content is ready

      // Return immediately to display main content
      if (searchResultsMatch || apiResponse.aircraft.length > 0) {
        return {
          response: apiResponse,
          mainContentReady: true,
          rightSideReady: departureAirportsMatch && arrivalAirportsMatch,
  ...
  // If we reach here, maxAttempts was reached without main content being displayed
...
// Function to poll specifically for right side content
async function pollForRightSideContent(
  flightRequestId,
  expectedDepartureAirports,
  expectedArrivalAirports,
  maxAttempts = 15, //!right_side
  apiUrl = "https://operators-dashboard.bubbleapps.io/api/1.1/wf/webflow_return_data_blackjet",
  interval = 1000
) {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (departureAirportsMatch && arrivalAirportsMatch) {
      return apiResponse;
    }
  ...
  // Return the latest response even if conditions aren't met
  const finalResponse = await fetch(apiUrl, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ flightrequest: flightRequestId }),
...
function renderRightSideContent(aircraftResponse) {
  ...
  if (departureArea && departureAirport) {
    ...
    departureAirport
      ...
      .forEach((depAirport) => {
        const isActive =
          getStoredData && getStoredData.fromId === depAirport.airport_id_text;
        departureArea.innerHTML += `
    ...
    document.querySelectorAll(".depair_item_click").forEach((item) => {
      item.addEventListener("click", function () {
        ...
        const selectedAirport = aircraftResponse.other_departure_airports.find(
  ...
  if (arrivalArea && arrivalAirport) {
    ...
    const seeMoreElement = document.querySelector(".ac_dep_see.seemore_arive");
  ...
  // Hide small loading box after right side content is successfully rendered
  const smLoadingBoxes = document.querySelectorAll(".sm_loading_box");
  smLoadingBoxes.forEach((box) => {
    box.style.display = "none";
  });

  // Handle "No Alternate Airports Nearby" display
  ...
  // Clear previous messages if data exists now
  if (deptopElem && !noDep) {
    const depMsg = deptopElem.querySelector(".no-airport");
    if (depMsg) depMsg.remove();
  }
  if (arrivetopElem && !noArr) {
    const arrMsg = arrivetopElem.querySelector(".no-airport");
    if (arrMsg) arrMsg.remove();
  }

  // If none available, show message(s) without duplicating
  if (deptopElem && noDep) {
    if (!deptopElem.querySelector(".no-airport")) {
      deptopElem.innerHTML = `<p class="no-airport">No Alternate Airports Nearby</p>`;
    }
  }
  if (arrivetopElem && noArr) {
    if (!arrivetopElem.querySelector(".no-airport")) {
      arrivetopElem.innerHTML = `<p class="no-airport">No Alternate Airports Nearby</p>`;
    }
  }

  // Arrow visibility: show if either side has items, hide only if both empty
  if (arrowIcon) {
    arrowIcon.style.display = noDep && noArr ? "none" : "";
  }
}
...
// Function to fire Klaviyo product view tracking for logged-in users
function fireKlaviyoProductViewTracking(aircraftData) {
  try {
    ...
    if (aircraftData && aircraftData.length > 0) {
      // Find the cheapest aircraft for tracking
      const cheapestAircraft = aircraftData.reduce((cheapest, current) => {
        return current.price_number < cheapest.price_number
          ? current
          : cheapest;
      });

      // Fire Klaviyo product view event
      if (
        typeof window.klaviyo !== "undefined" &&
        typeof window.klaviyo.track === "function"
      ) {
        const klaviyoProduct = {
          ProductName: cheapestAircraft.category_text || "",
          ProductID: cheapestAircraft._id || "",
          SKU: cheapestAircraft._id || "",
          Categories: cheapestAircraft.category_text || "",
          ImageURL: `https:${cheapestAircraft.aircraft_image_image}`,
          URL: `https://blackjet.com/fleet/${cheapestAircraft.Slug}` || "",
          Brand: cheapestAircraft.operator_txt_text || "",
          Price: Math.round(cheapestAircraft.price_number),
          CompareAtPrice: Math.round(cheapestAircraft.price_number),
          Title: cheapestAircraft.category_text || "Aircraft",
          ItemId: cheapestAircraft._id || "",
          Metadata: {
            Brand: cheapestAircraft.operator_txt_text || "",
            Price: Math.round(cheapestAircraft.price_number),
            CompareAtPrice: Math.round(cheapestAircraft.price_number),
        ...
        window.klaviyo.track("Viewed Product", klaviyoProduct);
      }
    }
  } catch (error) {
    console.error("Error firing Klaviyo product view tracking:", error);
  }
}
...
async function makeApiCall() {
  try {
    ...
    if (flightRequestId && !storedFlightIds.includes(flightRequestId)) {
      ...
      //!Save flightrequestId in sessionstorage as an array for other api call
      if (flightRequestId) {
        sessionStorage.setItem("fRequestIdForApi", flightRequestId);
      }
    ...
    const pollResult = await pollForAircraft(
      flightRequestId,
    ...
    const aircraftResponse = pollResult.response;
    const rightSideReady = pollResult.rightSideReady;
    const attemptsUsed = pollResult.attemptsUsed || 0;

    // Render main content immediately (aircraft list, map, etc.)

    // Continue with right side polling if needed
    if (!pollResult.rightSideReady) {
      // Show small loading box for right side polling
      const smLoadingBoxes = document.querySelectorAll(".sm_loading_box");
      smLoadingBoxes.forEach((box) => {
        box.style.display = "block";
      });

      // Calculate remaining attempts (30 - attempts already made for main content)
      const remainingAttempts = 15 - attemptsUsed; //!right_side

      // Start right side polling in the background (don't await it)
      pollForRightSideContent(
        flightRequestId,
        apiResponse["Expected Alternate Departure Airports"],
        apiResponse["Expected Alternate Arrival Airports"],
        remainingAttempts
      ).then((rightSideResult) => {
        if (rightSideResult) {
          // Update the aircraft response with the latest right side data
          aircraftResponse.other_departure_airports =
            rightSideResult.other_departure_airports;
          aircraftResponse.other_arrival_airports =
            rightSideResult.other_arrival_airports;

          // Re-render the right side content with updated data
          renderRightSideContent(aircraftResponse);
        } else {
          // Hide small loading box if no result (polling completed but no data)
          const smLoadingBoxes = document.querySelectorAll(".sm_loading_box");
          smLoadingBoxes.forEach((box) => {
            box.style.display = "none";
          });
      ...
      // If right side content is already ready, hide the small loading box
      const smLoadingBoxes = document.querySelectorAll(".sm_loading_box");
      smLoadingBoxes.forEach((box) => {
        box.style.display = "none";
      });
    ...
    // Fire Klaviyo product view tracking for logged-in users when aircraft data loads
    fireKlaviyoProductViewTracking(apiAircraft);
...
`;
    ...
    // Only render right side content if it's ready, otherwise wait for polling to complete
    if (pollResult.rightSideReady) {
      renderRightSideContent(aircraftResponse);
    }
    ...
    // Note: Right side availability messaging handled inside renderRightSideContent
