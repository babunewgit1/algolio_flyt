...
function setActiveItems() {
  ...
  // Country items
  const countryItems = currencyMenu?.querySelectorAll(".cl_list_wrapper .clist_item");
  countryItems?.forEach(function (item, index) {
    const code = item.getAttribute("data-short-cl");
    const isMatch = savedCountry ? savedCountry.shortCode === code : index === 0;
    item.classList.toggle("active", isMatch);
  });

  // Currency items
  const currencyItems = currencyMenu?.querySelectorAll(".carrency_list_wrapper .clist_item");
  currencyItems?.forEach(function (item, index) {
    const code = item.getAttribute("data-short-currency");
    const isMatch = savedCurrency ? savedCurrency.shortCode === code : index === 0;
    item.classList.toggle("active", isMatch);
  });
}
...
countryItems?.forEach(function (item) {
  item.addEventListener("click", function () {
    ...
    sessionStorage.setItem("country", JSON.stringify({ name: name, shortCode: shortCode }));
    location.reload();
  });
});
...
currencyItems?.forEach(function (item) {
  item.addEventListener("click", function () {
    ...
    sessionStorage.setItem("currency", JSON.stringify({ name: name, shortCode: shortCode, api_currency: apiCurrency }));
    location.reload();
  });
});
